...
export default function RegisterVeterinario() {
    ...
    console.log('Datos a enviar:', doctorGuardar);
    ...
    const validateSpecilidad = () => {
        let newErrors = {};
        if (!doctorGuardar.id_institucion_vet_1) newErrors.id_institucion_vet_1 = "La calle es requerida";
        if (!doctorGuardar.id_institucion_vet_1) newErrors.id_institucion_vet_1 = "El número es requerido";
        setErrors(newErrors);
        return Object.keys(newErrors).length === 0;
    };
    ...
    const handleManualFinish = async () => {
        if (validateDireccionManual()) {
            try {
                ...
                console.log('Datos enviados:', veterinarioJSON);
                const response = await VeterinarioService.guardarDoctor(doctorGuardar);
    ...
    const cardDatosVet = (
        <Fragment>
            <CardContent>
                ...
                <Grid item xs={12}>
                    <TextField
                        id="nombre-institucion" label="Nombre Institución" variant="outlined" fullWidth required
                        value={doctorGuardar.id_institucion_vet_1} onChange={(e) => cambiarCampo("id_institucion_vet_1", e.target.value)}
                    />
                </Grid>
                <Grid container spacing={2} sx={{ mt: 1 }} alignItems="center">
                    <Grid item>
                    ...
                    <Grid item xs={12}>
                        <FormControl fullWidth>
                            ...
                            <Select
                                ...
                                id="specialty-select"
                                value={doctorGuardar.id_institucion_vet_1}
                                label="id_institucion_vet_1"
                                onChange={(e) => cambiarCampo("id_institucion_vet_1", e.target.value)}
                            >
